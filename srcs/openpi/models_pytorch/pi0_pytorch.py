...
class PI0Pytorch(nn.Module):
    def __init__(self, config):
        ...
        logging.info("PI0Pytorch: init")  # === [ADDED] ===
        ...
        logging.info("PI0Pytorch: init complete")  # === [ADDED] ===
    ...
    def embed_prefix(
        ...
        """Embed images with SigLIP + VGGT fusion and language tokens to prepare transformer inputs."""
        embs = []
        pad_masks = []
        att_masks = []
        ...
        for img, img_mask, spl_emb in zip(images, img_masks, spatial_tokens, strict=True):
    ...
    # [ADDED] Shared backbone embedding extractor for critic (masked mean pool over prefix tokens)
    @torch.no_grad()
    def extract_vlm_embedding(self, observation, *, train=False) -> torch.Tensor:
        """
        Returns h_t: pooled multimodal embedding (B, width).
        Uses only prefix (image + language) tokens; backbone frozen usage.
        """
        images, img_masks, lang_tokens, lang_masks, _state_unused = self._preprocess_observation(observation, train=train)
        prefix_embs, prefix_pad_masks, _prefix_att_masks = self.embed_prefix(images, img_masks, lang_tokens, lang_masks)
        # prefix_pad_masks: bool (B, N)
        mask = prefix_pad_masks
        # Safe float masking
        mask_f = mask.to(prefix_embs.dtype)
        denom = mask_f.sum(dim=1, keepdim=True).clamp_min(1.0)
        pooled = (prefix_embs * mask_f.unsqueeze(-1)).sum(dim=1) / denom
